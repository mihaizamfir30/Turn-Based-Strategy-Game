...
public class HexCellShaderData : MonoBehaviour {
	...
	public void GetVisibilityFromPlayer(HexPlayer player)
    {
        for (int i = 0; i < player.cellVisibilityData.Length; i += 2)
        {
			cellTextureData[i / 2].r = player.cellVisibilityData[i] ? (byte)255 : (byte)0;
			cellTextureData[i / 2].g = player.cellVisibilityData[i + 1] ? (byte)255 : (byte)0;

		}
    }

	public void SetVisibilityForPlayer(HexPlayer player)
    {

        for (int i = 0; i < cellTextureData.Length; i++)
        {
			player.cellVisibilityData[2 * i + 0] = cellTextureData[i].r == (byte)255 ? true : false;
			player.cellVisibilityData[2 * i + 1] = cellTextureData[i].g == (byte)255 ? true : false;
		}
